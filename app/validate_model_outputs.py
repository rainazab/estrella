...
from .config import BASE_DIR, LINES, OUTPUT_DIR
from .export_data_json import build_line_baseline, evidence_quality_label
...
def _validate_recommendations(problems: list[str], data: dict[str, Any]) -> None:
    ...
    for rec_key, rec in (data.get("recommendations") or {}).items():
        ...
        _assert(
            problems,
            rec.get("evidenceStrengthLabel") == expected_quality,
            f"recommendations.{rec_key}.evidenceStrengthLabel must be {expected_quality!r}",
        )
        _assert(
            problems,
        ...
        if analogue_mean is not None and naive_mean is not None and gain is not None:
            ...
            comparison = evidence.get("oeeComparison") or {}
            _assert(
                problems,
                comparison.get("metric") == "comparative_oee_points",
                f"recommendations.{rec_key}.evidence.oeeComparison must declare comparative_oee_points",
            )
            comparison_gain = _parse_float(comparison.get("gainPoints"))
            _assert(
                problems,
                comparison_gain is not None and abs(comparison_gain - gain) < 0.2,
                f"recommendations.{rec_key}.evidence.oeeComparison.gainPoints does not match evidence.gain",
            )
        _assert(
            problems,
